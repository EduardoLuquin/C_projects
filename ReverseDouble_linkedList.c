...
{
     /*Since here, my implementation*/
    int nlsize=1; 
    DoublyLinkedList* lrev = malloc(sizeof(DoublyLinkedList));
    lrev->head = NULL;
    lrev->tail = NULL;
    
    while(llist->next)
    {
        printf("Normal List: %d \n",llist->data);
        llist = llist->next;
        nlsize++; 
    }
    
    //llist = llist->prev->prev; 
    printf("Normal List: %d \n",llist->data);
    printf(" ******************* \n");
    
    
    //Build reverse list
    for(int it=0; it<nlsize; it++)
    {
         insert_node_into_doubly_linked_list(&lrev, llist->data);
         printf("Reverse List: %d \n",llist->data);
         llist = llist->prev; 
    }
    
    printf("List Head: %d/n", lrev->head->data); 
